...
# DEFAULT_BANK_LEDGER = "Bank of Baroda"
...
#     # NEW: We pass the opening balance specifically to the Bank Ledger
...
#     # NEW: Inject Opening Balance if provided
#     if opening_balance is not None:
#         # If opening balance is positive (Dr), Tally expects a negative string
#         # If opening balance is negative (Cr), Tally expects a positive string
#         tally_op_bal = -opening_balance
#         ET.SubElement(ledger, "OPENINGBALANCE").text = f"{tally_op_bal:.2f}"


# def _add_voucher(request_data, transaction, index, bank_ledger, suspense_ledger):
#     # Book View Analogy: 
#     # CREDIT transaction -> Money out of bank -> Payment Voucher
#     # DEBIT transaction  -> Money into bank  -> Receipt Voucher  
#     voucher_type = "Payment" if transaction["type"] == "CREDIT" else "Receipt"
#     amount = round(float(transaction["amount"]), 2)
#     date_value = _format_tally_date(transaction["gl_date"])
#     narration = transaction.get("narration") or "Bank Statement Entry"
#     voucher_number = _voucher_number(transaction, index)
...
#     if transaction["type"] == "CREDIT":
#         # Payment: Cash leaves the Bank. 
#         # Bank Ledger is Credited (is_debit=False), Suspense is Debited (is_debit=True)
#         _add_ledger_entry(voucher, bank_ledger, amount, is_debit=False, is_party_ledger=False)
#         _add_ledger_entry(voucher, suspense_ledger, amount, is_debit=True, is_party_ledger=True)
#     else:
#         # Receipt: Cash enters the Bank. 
#         # Bank Ledger is Debited (is_debit=True), Suspense is Credited (is_debit=False)
#         _add_ledger_entry(voucher, suspense_ledger, amount, is_debit=False, is_party_ledger=True)
#         _add_ledger_entry(voucher, bank_ledger, amount, is_debit=True, is_party_ledger=False)
...
# def _voucher_number(transaction, index):
#     date_part = _format_tally_date(transaction["gl_date"])
#     return f"BOB-{date_part}-{index:05d}"
...
def generate_tally_xml(
    ...
    # ─── CRITICAL TIMELINE SORTING FIX ─────────────────────────────────────
    # Convert dates to raw datetime values to sort chronologically,
    # ensuring dates like 10 Apr don't cut off lines past 9 Apr.
    ...
    # Extract a dynamic acronym prefix (e.g., "BANK OF BARODA" -> "BOB", "STATE BANK OF INDIA" -> "SBI")
    ...
    for index, transaction in enumerate(sorted_transactions, start=1):
        ...
        _add_voucher(
            ...
            bank_ledger,
            suspense_ledger,
            bank_prefix,  # Pass dynamic ledger acronym down
        )
...
def _add_voucher(request_data, transaction, index, bank_ledger, suspense_ledger, bank_prefix):
    ...
    # Generate dynamic prefix IDs (SBI-20250401-00001) instead of hardcoded BOB rules
